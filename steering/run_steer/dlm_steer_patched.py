...
def set_current_diffusion_step(t: int):
    global CURRENT_DIFFUSION_STEP
    CURRENT_DIFFUSION_STEP = t


def reset_current_diffusion_step():
    global CURRENT_DIFFUSION_STEP
    CURRENT_DIFFUSION_STEP = 0


def in_step_window(t: int, T: int, mode: str) -> bool:
    if mode == "early":
        return t < T // 3
    elif mode == "middle":
        return T // 3 <= t < 2 * T // 3
        ...
        raise ValueError(f"Unknown step window mode: {mode}")
...
def parse_args():
    ...
    ap.add_argument('--steer_mode', type=str, default="amplify",
                    choices=["amplify", "zero"],
                    help="Amplify selected feature(s) or zero them out.")
    ap.add_argument('--step_window', type=str, default="all",
                    choices=["all", "early", "middle", "late"],
                    help="Apply steering only in a selected diffusion-step window.")
...
class ZeroFeatureSAEHookDLM:
    """
    Forward hook for DLM layers:
    - Works on [B, D] (expanded) or [B, T, D] tensors.
    - Zeros selected SAE features across positions.
    - Residual error compensation: add (hidden - clean_recon) back to preserve baseline content.
    - Optionally restricts steering to a diffusion-step window.
    """
    def __init__(self, sae, feature_ids, device: str,
                 total_steps: int,
                 step_window: str = "all") -> None:
        self.sae = sae
        self.features = [int(f) for f in feature_ids]
        self.device = torch.device(device)
        self.total_steps = int(total_steps)
        self.step_window = step_window
        self._call_counter = 0  # fallback approximation if explicit step updates are unavailable

    def __call__(self, module, args, output):
        step_idx = CURRENT_DIFFUSION_STEP if CURRENT_DIFFUSION_STEP != 0 else self._call_counter
        self._call_counter += 1

        if not in_step_window(step_idx, self.total_steps, self.step_window):
            return output
        ...
        with torch.no_grad():
            clean_acts = self.sae.encode(hidden)
            clean_rec = self.sae.decode(clean_acts)
...
def _register_dlm_hook(model, sae, layer: int, feature: int, device: str,
                       ...
                       steer_mode: str = "amplify",
                       total_steps: int = 20,
                       step_window: str = "all"):
    """
    Register the DLM steering hook on the given Transformer block index (resid_post).
    """
    if steer_mode == "amplify":
...
def _run_dlm_steering_for_one_file(
    ...
    print(f"[INFO] steer_mode={steer_mode} step_window={step_window}")
...
# Build model
# -----------------------------
# old function but when i was running it i came across this error:
#Traceback (most recent call last):
#   File "/content/SAE4DLM-CE/steering/run_steer/dlm_steer.py", line 982, in <module>
#     main()
#   File "/content/SAE4DLM-CE/steering/run_steer/dlm_steer.py", line 905, in main
#     model, tokenizer = _build_dlm_model(args.model_name, dtype, args.device, args.trust_remote_code)
#                        ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#   File "/content/SAE4DLM-CE/steering/run_steer/dlm_steer.py", line 870, in _build_dlm_model
#     model = AutoModel.from_pretrained(
#             ^^^^^^^^^^^^^^^^^^^^^^^^^^
#   File "/usr/local/lib/python3.12/dist-packages/transformers/models/auto/auto_factory.py", line 559, in from_pretrained
#     return model_class.from_pretrained(
#            ^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#   File "/root/.cache/huggingface/modules/transformers_modules/dream-7b-local/modeling_dream.py", line 565, in from_pretrained
#     _model = super().from_pretrained(
#              ^^^^^^^^^^^^^^^^^^^^^^^^
#   File "/usr/local/lib/python3.12/dist-packages/transformers/modeling_utils.py", line 4097, in from_pretrained
#     model = cls(config, *model_args, **model_kwargs)
#             ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# TypeError: DreamModel.__init__() got an unexpected keyword argument 'dtype'
